...
(defn wabbit
  ...
  (try (main/info (format "Generating fresh 'lein new' %s project." ws/*template-name*))
       (let [options {:renderer-fn sc/render-string
                      :dir (or *dir*
                               (-> "leiningen.original.pwd"
                                   System/getProperty (io/file name) .getPath))
                      :force? *force?*
